...
bl <- read_tsv(here("00_input", "HPA_annotations.tsv"), show_col_types = FALSE) |>
  ...
  mutate(
    ...
    muscle_keep    = !is.na(myo) & myo >= 50,      # myonuclei nCPM at/above this rescues as real muscle
    reason = case_when(
...
ann_lookup <- raw |> as.data.frame() |> distinct(gene, .keep_all = TRUE) |>
...
removed <- bl |> filter(verdict == "remove", gene %in% raw$gene) |>
  ...
  transmute(gene, uniprot, protein, description, reason,
            secretome, blood_conc, erythrocyte_nCPM = ery, myonuclei_nCPM = myo) |>
  arrange(reason, desc(blood_conc))

# blood-derived proteins KEPT by the myonuclei rescue, for review
rescued <- bl |> filter(str_detect(reason, "rescued"), gene %in% raw$gene) |>
  left_join(ann_lookup, by = "gene") |>
  transmute(gene, uniprot, protein, description, reason,
            secretome, blood_conc, erythrocyte_nCPM = ery, myonuclei_nCPM = myo) |>
...
write.xlsx(
  list(filter_log           = flog,
       ...
       blood_classification = bl |> select(gene, uniprot, protein_class, secretome,
