# F01 CR Panel A: STS Max Power (pre/post + change by supplement)
...
# CR subjects only, T1 row, complete pre+post for STS
subj <- meta %>%
  filter(timepoint == "T1", cancer == "SURV",
         !is.na(pre_sts_max_pwr), !is.na(post_sts_max_pwr)) %>%
  mutate(supp = factor(supp, levels = c("CRE", "PLA")),
         delta_sts = post_sts_max_pwr - pre_sts_max_pwr)

# Long form for ANOVA + bar plots
subj_long <- subj %>%
  select(pid, supp, pre_sts_max_pwr, post_sts_max_pwr) %>%
  pivot_longer(cols = c(pre_sts_max_pwr, post_sts_max_pwr),
               names_to = "time", values_to = "sts") %>%
...
# Paired t-tests within each supplement
cre_subj <- subj %>% filter(supp == "CRE")
pla_subj <- subj %>% filter(supp == "PLA")
stats_paired_cre <- t.test(cre_subj$post_sts_max_pwr,
                           cre_subj$pre_sts_max_pwr, paired = TRUE)
stats_paired_pla <- t.test(pla_subj$post_sts_max_pwr,
                           pla_subj$pre_sts_max_pwr, paired = TRUE)
stats_delta      <- t.test(delta_sts ~ supp, data = subj)
...
sw_cre <- shapiro.test(cre_subj$delta_sts)
sw_pla <- shapiro.test(pla_subj$delta_sts)
...
write.csv(audit_A, file.path(DAT, "panel_A_sts.csv"), row.names = FALSE)

# --- Left plot: Absolute pre/post by supplement
y_max_left <- max(subj_long$sts, na.rm = TRUE)

pA_left <- ggplot(subj_long, aes(x = supp_time, y = sts, fill = supp_time)) +
  ...
  labs(title = "STS Max Power", subtitle = full_sub,
       y = "STS Max Power (W)", x = NULL, tag = "A") +
  FIG_THEME +
...
y_max_right <- max(subj$delta_sts, na.rm = TRUE)

pA_right <- ggplot(subj, aes(x = supp, y = delta_sts, fill = supp)) +
  ...
  labs(y = expression(Delta * " STS Max Power (W)"), x = NULL) +
  FIG_THEME + theme(legend.position = "none")

pA <- (pA_left | pA_right) + plot_layout(widths = c(0.65, 0.35))

ggsave(file.path(RPT, "panel_A_sts.pdf"), pA,
       width = PW, height = PH, units = "mm", device = get_pdf_device())
ggsave(file.path(RPT, "panel_A_sts.png"), pA,
       width = PW, height = PH, units = "mm", dpi = 300)
cat("F01 CR Panel A done\n")
