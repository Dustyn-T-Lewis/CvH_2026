# F01 CR Panel C: ALM (pre/post + change by supplement)
...
# CR subjects only, T1 row, complete pre+post for ALM
subj <- meta %>%
...
stats_anova <- rstatix::anova_test(data = subj_long, dv = alm,
                                    wid = pid,
...
stats_delta      <- t.test(delta_alm ~ supp, data = subj)
...
write.csv(audit_C, file.path(DAT, "panel_C_alm.csv"), row.names = FALSE)

# --- Left plot: Absolute pre/post by supplement
y_max_left <- max(subj_long$alm, na.rm = TRUE)

pC_left <- ggplot(subj_long, aes(x = supp_time, y = alm, fill = supp_time)) +
...
y_max_right <- max(subj$delta_alm, na.rm = TRUE)

pC_right <- ggplot(subj, aes(x = supp, y = delta_alm, fill = supp)) +
...
ggsave(file.path(RPT, "panel_C_alm.pdf"), pC,
       width = PW, height = PH, units = "mm", device = get_pdf_device())
ggsave(file.path(RPT, "panel_C_alm.png"), pC,
       width = PW, height = PH, units = "mm", dpi = 300)
cat("F01 CR Panel C done\n")
