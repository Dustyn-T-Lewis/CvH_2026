# F01 CR Supplementary Panel D: Chest Press (pre/post + change by supplement)
...
PW <- 170; PH <- 80
RPT <- "04_Figures/F01/CR/b_reports/supp"
...
# CR subjects only, T1 row, complete pre+post for chest press
subj <- meta %>%
  filter(timepoint == "T1", cancer == "SURV",
         !is.na(pre_chest_press_lbs), !is.na(post_chest_press_lbs)) %>%
  mutate(supp = factor(supp, levels = c("CRE", "PLA")),
         delta_cp = post_chest_press_lbs - pre_chest_press_lbs)

# Long form for ANOVA + bar plots
subj_long <- subj %>%
  select(pid, supp, pre_chest_press_lbs, post_chest_press_lbs) %>%
  pivot_longer(cols = c(pre_chest_press_lbs, post_chest_press_lbs),
               names_to = "time", values_to = "cp") %>%
...
stats_anova <- rstatix::anova_test(data = subj_long, dv = cp,
                                    wid = pid,
                                    between = supp, within = time)

cre_subj <- subj %>% filter(supp == "CRE")
pla_subj <- subj %>% filter(supp == "PLA")
stats_paired_cre <- t.test(cre_subj$post_chest_press_lbs,
                           cre_subj$pre_chest_press_lbs, paired = TRUE)
stats_paired_pla <- t.test(pla_subj$post_chest_press_lbs,
                           pla_subj$pre_chest_press_lbs, paired = TRUE)
stats_delta      <- t.test(delta_cp ~ supp, data = subj)
...
sw_cre <- shapiro.test(cre_subj$delta_cp)
sw_pla <- shapiro.test(pla_subj$delta_cp)
...
write.csv(audit_D, file.path(DAT, "panel_D_chest_press.csv"), row.names = FALSE)

# --- Left plot: Absolute pre/post by supplement
y_max_left <- max(subj_long$cp, na.rm = TRUE)

pD_left <- ggplot(subj_long, aes(x = supp_time, y = cp, fill = supp_time)) +
  ...
  labs(title = "Chest Press", subtitle = full_sub,
       y = "Chest Press (lbs)", x = NULL, tag = "D") +
  FIG_THEME +
...
y_max_right <- max(subj$delta_cp, na.rm = TRUE)

pD_right <- ggplot(subj, aes(x = supp, y = delta_cp, fill = supp)) +
  ...
  labs(y = expression(Delta * " Chest Press (lbs)"), x = NULL) +
  FIG_THEME + theme(legend.position = "none")

pD <- (pD_left | pD_right) + plot_layout(widths = c(0.65, 0.35))

ggsave(file.path(RPT, "panel_D_chest_press.pdf"), pD,
       width = PW, height = PH, units = "mm", device = get_pdf_device())
ggsave(file.path(RPT, "panel_D_chest_press.png"), pD,
       width = PW, height = PH, units = "mm", dpi = 300)
cat("F01 CR Supp Panel D done\n")
