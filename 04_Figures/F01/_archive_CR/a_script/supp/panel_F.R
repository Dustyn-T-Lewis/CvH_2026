# F01 CR Supplementary Panel F: Grip Strength (pre/post + change by supplement)
...
# CR subjects only, T1 row, complete pre+post for grip
subj <- meta %>%
  filter(timepoint == "T1", cancer == "SURV",
         !is.na(pre_grip_lbs), !is.na(post_grip_lbs)) %>%
  mutate(supp = factor(supp, levels = c("CRE", "PLA")),
         delta_grip = post_grip_lbs - pre_grip_lbs)

# Long form for ANOVA + bar plots
subj_long <- subj %>%
  select(pid, supp, pre_grip_lbs, post_grip_lbs) %>%
  pivot_longer(cols = c(pre_grip_lbs, post_grip_lbs),
               names_to = "time", values_to = "grip") %>%
...
stats_anova <- rstatix::anova_test(data = subj_long, dv = grip,
                                    wid = pid,
                                    between = supp, within = time)

cre_subj <- subj %>% filter(supp == "CRE")
pla_subj <- subj %>% filter(supp == "PLA")
stats_paired_cre <- t.test(cre_subj$post_grip_lbs,
                           cre_subj$pre_grip_lbs, paired = TRUE)
stats_paired_pla <- t.test(pla_subj$post_grip_lbs,
                           pla_subj$pre_grip_lbs, paired = TRUE)
stats_delta      <- t.test(delta_grip ~ supp, data = subj)
...
sw_cre <- shapiro.test(cre_subj$delta_grip)
sw_pla <- shapiro.test(pla_subj$delta_grip)
...
write.csv(audit_F, file.path(DAT, "panel_F_grip.csv"), row.names = FALSE)

# --- Left plot: Absolute pre/post by supplement
y_max_left <- max(subj_long$grip, na.rm = TRUE)

pF_left <- ggplot(subj_long, aes(x = supp_time, y = grip, fill = supp_time)) +
  ...
  labs(title = "Grip Strength", subtitle = full_sub,
       y = "Grip Strength (lbs)", x = NULL, tag = "F") +
  FIG_THEME +
...
y_max_right <- max(subj$delta_grip, na.rm = TRUE)

pF_right <- ggplot(subj, aes(x = supp, y = delta_grip, fill = supp)) +
  ...
  labs(y = expression(Delta * " Grip Strength (lbs)"), x = NULL) +
  FIG_THEME + theme(legend.position = "none")

pF <- (pF_left | pF_right) + plot_layout(widths = c(0.65, 0.35))

ggsave(file.path(RPT, "panel_F_grip.pdf"), pF,
       width = PW, height = PH, units = "mm", device = get_pdf_device())
ggsave(file.path(RPT, "panel_F_grip.png"), pF,
       width = PW, height = PH, units = "mm", dpi = 300)
cat("F01 CR Supp Panel F done\n")
