# F01 CR Supplementary Panel E: Leg Extension (pre/post + change by supplement)
...
# CR subjects only, T1 row, complete pre+post for leg ext
subj <- meta %>%
...
write.csv(audit_E, file.path(DAT, "panel_E_leg_ext.csv"), row.names = FALSE)

# --- Left plot: Absolute pre/post by supplement
y_max_left <- max(subj_long$le, na.rm = TRUE)

pE_left <- ggplot(subj_long, aes(x = supp_time, y = le, fill = supp_time)) +
  ...
  labs(title = "Leg Extension", subtitle = full_sub,
       y = "Leg Extension (lbs)", x = NULL, tag = "E") +
  FIG_THEME +
...
pE_right <- ggplot(subj, aes(x = supp, y = delta_le, fill = supp)) +
...
ggsave(file.path(RPT, "panel_E_leg_ext.pdf"), pE,
       width = PW, height = PH, units = "mm", device = get_pdf_device())
ggsave(file.path(RPT, "panel_E_leg_ext.png"), pE,
       width = PW, height = PH, units = "mm", dpi = 300)
cat("F01 CR Supp Panel E done\n")
