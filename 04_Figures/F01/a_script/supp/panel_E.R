...
write.csv(audit_E, file.path(DAT, "panel_E_chest_press.csv"), row.names = FALSE)

y_max_left <- max(subj_long$cp, na.rm = TRUE)

pE_left <- ggplot(subj_long, aes(x = supp_time, y = cp, fill = supp_time)) +
  ...
  labs(title = "Chest Press", subtitle = full_sub,
       y = "Chest Press (lbs)", x = NULL, tag = "E") +
  FIG_THEME +
...
pE_right <- ggplot(subj, aes(x = supp, y = delta_cp, fill = supp)) +
...
ggsave(file.path(RPT, "panel_E_chest_press_SUPP.pdf"), pE,
       width = PW, height = PH, units = "mm", device = get_pdf_device())
ggsave(file.path(RPT, "panel_E_chest_press_SUPP.png"), pE,
