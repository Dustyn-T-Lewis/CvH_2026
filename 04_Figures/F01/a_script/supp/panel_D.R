...
write.csv(audit_D, file.path(DAT, "panel_D_lbm.csv"), row.names = FALSE)

y_max_left <- max(subj_long$lbm, na.rm = TRUE)

pD_left <- ggplot(subj_long, aes(x = supp_time, y = lbm, fill = supp_time)) +
...
y_max_right <- max(abs(subj$delta_lbm), na.rm = TRUE)

pD_right <- ggplot(subj, aes(x = supp, y = delta_lbm, fill = supp)) +
...
ggsave(file.path(RPT, "panel_D_lbm_SUPP.pdf"), pD,
       width = PW, height = PH, units = "mm", device = get_pdf_device())
ggsave(file.path(RPT, "panel_D_lbm_SUPP.png"), pD,
