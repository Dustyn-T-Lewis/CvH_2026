# Figure 2 (CRvH) — Panel E: Intra-Individual Proteomic Variability (Imputed)
# One boxplot per subject, faceted by Supplement (CRE | PLA), ordered by median log2FC.
# Filtered to CR subjects only (those with paired T1/T2).
...
# Filter to CR subjects (CR_CRE, CR_PLA) with paired T1/T2
meta <- meta_full |>
  filter(Group %in% c("CR_CRE", "CR_PLA"), Col_ID %in% samp_names)

# Keep only subjects with both T1 and T2
...
# ── Compute per-subject log2FC (T2 - T1) ──
