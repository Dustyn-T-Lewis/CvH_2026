# Figure 2 (CRvH) — Supp Panel C (Non-Imputed): Intra-Individual Proteomic Variability
# Complete-case logFC only (both T1 and T2 observed). No imputation.
# Filtered to CR subjects with paired T1/T2, faceted by Supplement (CRE | PLA).
...
# ── Load non-imputed normalized data ──
...
# Filter to CR subjects with paired T1/T2
meta <- meta_full |>
...
# ── Compute complete-case logFC per subject ──
lfc_list <- lapply(subjects, function(s) {
  ...
  supplement <- meta$Supplement[meta$Subject_ID == s][1]
