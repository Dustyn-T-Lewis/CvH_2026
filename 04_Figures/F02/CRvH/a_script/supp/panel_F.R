...
# --- Blood markers ---
blood_markers <- c("HBB", "HBA1", "HBA2", "ALB", "TF", "HP", "SERPINA1",
...
n_p <- nrow(plasma_df); r_p <- cor(plasma_df$cv_cr, plasma_df$cv_h, use = "complete.obs")
ci_p <- fisher_z_ci(r_p, n_p)

n_np <- nrow(nonplasma_df); r_np <- cor(nonplasma_df$cv_cr, nonplasma_df$cv_h, use = "complete.obs")
...
pF <- ggplot(scatter_df, aes(x = cv_h, y = cv_cr)) +
  ...
  geom_point(data = scatter_df |> filter(!is_plasma),
...
# --- Save ---
ggsave(file.path(RPT, "panel_F_cv_scatter_baseline_SUPP.png"), pF,
       width = PF_W, height = PF_H, units = "mm", dpi = 300)

write_csv(
  scatter_df |> dplyr::select(gene, cv_cr, cv_h, delta_cv, is_plasma, protein_class, subcellular_main),
  file.path(DAT, "audit_panel_F_cv_scatter.csv")
)
...
write_csv(
  ...
  file.path(DAT, "audit_panel_F_cor_stats.csv")
)
