...
# single module: protein count, member response (fGSEA NES over fry), phenotype
# association, eigengene trajectory with post-hoc brackets, and top-5 ORA.
...
# Member response: fGSEA NES tiles (top, modules as gene sets ranked by DE
# moderated-t) over fry tiles (below, signed -log10 q), per contrast, boxed at
# FDR < 0.05, with two diverging colorbars.
card_response <- function(nes, settests, modules) {
  ...
  nes_d <- nes |>
    ...
    transmute(module_color, contrast,
      row = "NES", value = NES,
      sig = if_else(!is.na(padj) & padj < 0.05, "FDR", "ns"),
      lab = sprintf("%.1f", NES)
    ) |>
    as_card()
  fry <- settests |>
    ...
    as_card()
  boxes <- bind_rows(nes_d, fry) |> filter(sig != "ns")

  ggplot(mapping = aes(contrast, row)) +
    geom_tile(
      data = nes_d, aes(fill = value), colour = "grey80",
      linewidth = 0.4, width = 0.96, height = 0.92
    ) +
    scale_fill_gradientn(
      colours = c("#1B7837", "white", "#762A83"), limits = c(-3.5, 3.5),
      oob = scales::squish, name = "fGSEA NES (up purple)",
      guide = guide_colorbar(
        order = 1, barwidth = unit(12, "mm"), barheight = unit(2, "mm"), title.vjust = 1
      )
    ) +
    ...
    scale_fill_gradient2(
      ...
      midpoint = 0, limits = c(-2, 2), oob = scales::squish, name = "fry ±log10 q",
      guide = guide_colorbar(
        order = 2, barwidth = unit(12, "mm"), barheight = unit(2, "mm"), title.vjust = 1
      )
    ) +
    geom_tile(
    ...
    geom_text(
      data = nes_d, aes(label = lab), size = 1.9,
      colour = if_else(abs(nes_d$value) > 2.5, "white", "grey15")
    ) +
    geom_text(
    ...
    scale_y_discrete(limits = c("fry", "NES"), labels = c(fry = "fry", NES = "NES")) +
    labs(
      title = "Member response", subtitle = "fGSEA NES (top) · fry (below)",
      x = NULL, y = NULL
    ) +
    FIG_THEME +
    card_facets() +
...
# Module eigengene vs baseline clinical outcome (Pearson r, BH-FDR box).
card_phenotype <- function(cor_pheno, modules) {
  d <- cor_pheno |>
    filter(module %in% modules, trait %in% names(PHENO_LABS)) |>
    mutate(
      module_color = factor(module, levels = modules),
      trait = factor(trait, levels = names(PHENO_LABS)),
      lab = sprintf("%.2f", r), sig = !is.na(padj) & padj < 0.05
    )
  ggplot(d, aes(trait, 1, fill = r)) +
    geom_tile(colour = "grey80", linewidth = 0.4, width = 0.96, height = 0.92) +
    geom_tile(
      data = filter(d, sig), fill = NA, colour = "black",
      linewidth = 0.9, width = 0.96, height = 0.92
    ) +
    geom_text(aes(label = lab),
      size = 1.7,
      colour = if_else(abs(d$r) > 0.6, "white", "grey15")
    ) +
    scale_fill_gradient2(
      low = "#1B7837", mid = "white", high = "#762A83", midpoint = 0,
      limits = c(-1, 1), name = "Pearson r",
      guide = guide_colorbar(barwidth = unit(12, "mm"), barheight = unit(2, "mm"))
    ) +
    scale_x_discrete(labels = PHENO_LABS) +
    scale_y_continuous(breaks = NULL, expand = c(0, 0)) +
    labs(
      title = "Phenotype", subtitle = "eigengene vs baseline (FDR box)",
      x = NULL, y = NULL
    ) +
    FIG_THEME +
    card_facets() +
    theme(
      ...
      axis.text.y = element_blank(), axis.ticks.y = element_blank(),
      legend.position = "bottom", legend.title = element_text(size = 6),
      legend.text = element_text(size = 5), plot.margin = margin(2, 2, 2, 2)
    )
}
...
assemble_card <- function(mod_bio, nes, settests, cor_pheno, traj, lmm, ora, modules) {
  wrap_plots(
    list(
      card_count(mod_bio, modules),
      card_response(nes, settests, modules),
      card_phenotype(cor_pheno, modules),
      card_trajectory(traj, lmm, modules),
      card_ora(ora, modules)
    ),
    nrow = 1, widths = c(0.5, 0.72, 0.75, 1, 1.7)
  )
}
