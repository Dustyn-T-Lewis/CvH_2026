# Figure 2 — Panel D (CR): fGSEA Grouped Bar Chart (Pathway Enrichment)
# Full collection: Hallmark + C2:CP (all subcollections) + GO:BP
# Per-database BH correction + Jaccard dedup.
# Reads CR result file for 3 contrasts.
# Outputs: pD_cr (ggplot object)
...
dep_df <- read_csv(DEP_FILE, show_col_types = FALSE) |>
  select(uniprot_id, gene, starts_with("t_"))

cat(sprintf("DEP (CR): %d proteins, %d contrasts\n", nrow(dep_df), length(CONTRASTS)))
...
write_csv(fgsea_export, file.path(DAT, "06_panel_H_CR_fgsea_results.csv"))
...
# Background rectangles for 3 contrasts
...
pD_cr <- ggplot(count_df, aes(x = contrast, y = fraction * 100, fill = direction)) +
  bg_rects +
  ...
  labs(title = "Pathway Enrichment (CR)",
...
write.csv(nes_summary, file.path(DAT, "panel_D_CR_nes_summary.csv"),
          row.names = FALSE)

# --- Save ---
ggsave(file.path(RPT, "panel_D_CR_fgsea.pdf"), pD_cr,
       width = PH_W, height = PH_H, units = "mm", device = pdf_device)
ggsave(file.path(RPT, "panel_D_CR_fgsea.png"), pD_cr,
       width = PH_W, height = PH_H, units = "mm", dpi = 300)

cat(sprintf("\nPanel D (CR) done: %d x %d mm, %d facets\n", PH_W, PH_H, n_facets))
