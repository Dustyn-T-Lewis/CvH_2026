# Figure 2 — Panel C (CR): UpSet Plot (Dual Bar + Dot Matrix)
# Outputs: pC_cr_bars, pC_cr_dots (ggplot objects)
# STANDALONE: generates sig_sets_cr, dir_map_cr, all_genes_cr locally (no panel_A dependency)
...
CONTRASTS <- c("Baseline_Supplement", "Training_CRE", "Training_PLA", "Supplement_Interaction")

# ============================================================================
# Generate required data locally (formerly from Panel A)
# ============================================================================
DEP_FILE <- "03_DEP/c_data/03_combined_results_CR.csv"
dep_df <- read_csv(DEP_FILE, show_col_types = FALSE)

SET_LABELS_cr <- CTR_SHORT[CONTRASTS]
all_genes_cr  <- unique(dep_df$gene[!is.na(dep_df$gene)])
...
for (ctr in CONTRASTS) {
  ...
  sig_sets_cr[[ctr]] <- dep_df$gene[is_sig]
  dir_map_cr[[ctr]]  <- setNames(ifelse(lfc_vals[is_sig] > 0, "Up", "Down"),
                                  dep_df$gene[is_sig])
}

pi_total_cr  <- sum(sapply(sig_sets_cr, length))
fdr_total_cr <- sum(sapply(CONTRASTS, function(ctr) {
  fdr_col <- paste0("adj.P.Val_", ctr)
  if (fdr_col %in% names(dep_df)) sum(dep_df[[fdr_col]] < 0.10, na.rm = TRUE) else 0
}))

SET_DISPLAY_COLORS_cr <- setNames(
  unname(CONTRAST_COLORS[CONTRASTS]),
  unname(SET_LABELS_cr[CONTRASTS])
)
...
bin_mat <- sapply(sig_sets_cr, function(s) as.integer(all_genes_cr %in% s))
rownames(bin_mat) <- all_genes_cr
colnames(bin_mat) <- SET_LABELS_cr[colnames(bin_mat)]
label_to_contrast <- setNames(names(SET_LABELS_cr), SET_LABELS_cr)
...
for (i in seq_len(n_comb)) {
  ...
  gene_dirs <- sapply(members, function(g) {
    dirs <- sapply(active_contrasts, function(ctr) {
      if (g %in% names(dir_map_cr[[ctr]])) dir_map_cr[[ctr]][g] else NA
    })
...
set_display_order <- unname(SET_LABELS_cr[CONTRASTS])
...
stripe_fills <- adjustcolor(unname(SET_DISPLAY_COLORS_cr[set_y_levels]), alpha.f = 0.20)

bar_bg_list <- lapply(seq_len(n_int), function(i) {
  bits <- as.logical(as.integer(strsplit(comb_names_ord[i], "")[[1]]))
  if (sum(bits) != 1) return(NULL)
  tibble(xmin = i - 0.5, xmax = i + 0.5,
         fill = unname(SET_DISPLAY_COLORS_cr[set_order_ch[bits]]))
})
bar_bg <- bind_rows(compact(bar_bg_list))

lbl_sz <- scale_text(BASE_COUNT, PG_W)

pC_cr_bars <- ggplot(bar_long, aes(x, count, fill = direction)) +
  ...
  labs(y = "Intersection\nsize",
       title = "Contrast Overlap (CR)",
       subtitle = sprintf(
         "\u03A0 < 0.05 DEPs; %d \u03A0 / %d FDR total (%d mixed excl.)",
         pi_total_cr, fdr_total_cr, n_mixed
       ),
       tag = "C2") +
  FIG_THEME +
...
# --- Fisher enrichment tests for all pairwise overlaps ---
overlap_tests <- list()
contrast_pairs <- combn(CONTRASTS, 2, simplify = FALSE)
n_bg <- length(all_genes_cr)
for (pair in contrast_pairs) {
  a <- sig_sets_cr[[pair[1]]]; b <- sig_sets_cr[[pair[2]]]
...
write.csv(overlap_df, file.path(DAT, "panel_C_CR_overlap_enrichment.csv"), row.names = FALSE)

pC_cr_combined <- (pC_cr_bars / pC_cr_dots) + plot_layout(heights = c(0.80, 0.20))

ggsave(file.path(RPT, "panel_C_CR_upset.pdf"), pC_cr_combined,
       width = PG_W, height = 120, units = "mm", device = pdf_device)
ggsave(file.path(RPT, "panel_C_CR_upset.png"), pC_cr_combined,
