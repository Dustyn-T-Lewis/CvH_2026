# Figure 2 — Panel A (CR): DEPs per Contrast (Pseudo-log Stacked Bar)
# Outputs: pA_cr (ggplot object)
# Side-effects: sig_sets_cr, dir_map_cr, all_genes_cr, SET_LABELS_cr,
#               SET_DISPLAY_COLORS_cr, pi_total_cr, fdr_total_cr (used by Panel C)
...
n_total <- length(all_genes_cr)
pi_ci <- data.frame(
  contrast = CONTRASTS,
  n_sig    = sapply(sig_sets_cr, length),
  n_total  = n_total,
  pct      = 100 * sapply(sig_sets_cr, length) / n_total,
  ci_lo    = sapply(sig_sets_cr, function(s) 100 * binom.test(length(s), n_total)$conf.int[1]),
  ci_hi    = sapply(sig_sets_cr, function(s) 100 * binom.test(length(s), n_total)$conf.int[2])
)
...
FRAC_FILL <- c()
for (cname in names(SET_DISPLAY_COLORS_cr)) {
  col <- unname(SET_DISPLAY_COLORS_cr[cname])
...
pA_cr <- ggplot(frac_df, aes(x = contrast, y = pct, fill = fill_key)) +
  bg_rects +
  ...
  scale_y_continuous(trans = scales::pseudo_log_trans(sigma = 3, base = 10),
                     expand = expansion(mult = c(0, 0.08)),
                     breaks = c(0, 2, 5, 10)) +
  coord_flip() +
  labs(title = "DEPs per Contrast (CR Model)",
       subtitle = sprintf("Fraction of %s filtered proteins",
                          format(length(all_genes_cr), big.mark = ",")),
...
write.csv(pi_ci, file.path(DAT, "panel_A_CR_dep_fraction_ci.csv"), row.names = FALSE)

ggsave(file.path(RPT, "panel_A_CR_dep_counts.pdf"), pA_cr,
       width = PF_W, height = 80, units = "mm", device = pdf_device)
ggsave(file.path(RPT, "panel_A_CR_dep_counts.png"), pA_cr,
       width = PF_W, height = 80, units = "mm", dpi = 300)
