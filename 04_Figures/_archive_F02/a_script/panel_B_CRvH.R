# Figure 2 — Panel B (CRvH): DEP Rank Location (Barcode Plot)
...
# Outputs: panel_B_CRvH_barcode.pdf/.png
...
DESC_DOWN <- c(Cancer_vs_Healthy = "proteins lower in cancer vs healthy",
               Training_CR       = "proteins dec. with CR training")
DESC_UP   <- c(Cancer_vs_Healthy = "proteins higher in cancer vs healthy",
               Training_CR       = "proteins inc. with CR training")
...
ggsave(file.path(RPT, "panel_B_CRvH_barcode.pdf"), pB_CRvH,
       width = PD_W, height = PD_H, units = "mm", device = pdf_device)
ggsave(file.path(RPT, "panel_B_CRvH_barcode.png"), pB_CRvH,
       width = PD_W, height = PD_H, units = "mm", dpi = 300)

message("Panel B (barcode CRvH) done.")
