...
p_key <- ggplot(key_df, aes(x = x, y = y)) +
  geom_point(aes(fill = category), shape = 21, size = 2.5,
...
make_half_bars <- function(df, fill_color, side, ylim) {
  ...
  p <- ggplot(bars, aes(y = y)) +
    ...
    theme(panel.grid   = element_blank(),
          ...
          axis.title.x = if (!is_upper) element_text(size = 8, face = "bold",
                                                     margin = margin(t = 0, unit = "mm"))
                          else element_blank(),
...
# 4 half-bar panels
p_ul <- make_half_bars(ora_tl, scales::alpha(COMP_BLUE, 0.30), "left",
                        c(0, abs(ylim_range[2])))
p_ll <- make_half_bars(ora_bl, scales::alpha(COMP_RED, 0.30),  "left",
                        c(-abs(ylim_range[2]), 0))
p_ur <- make_half_bars(ora_tr, scales::alpha(COMP_RED, 0.30),  "right",
                        c(0, abs(ylim_range[2])))
p_lr <- make_half_bars(ora_br, scales::alpha(COMP_BLUE, 0.30), "right",
                        c(-abs(ylim_range[2]), 0))
...
r_spear  <- cor(scatter_df$logFC_CvH, scatter_df$logFC_TR, use = "complete.obs",
                method = "spearman")
...
composite <- p_ul + p_scatter + p_ur + p_ll + p_lr + p_key +
  ...
  plot_annotation(
    ...
    subtitle = sprintf("Threshold-free ORA | N = %d | %d DEPs (\u03a0 < 0.05) | %d enriched (FDR < 0.05) | \u03c1 = %.2f",
...
pA_subtitle <- sprintf("N = %d | %d DEPs (\u03a0) | %d enriched | \u03c1 = %.2f",
                        n_total, n_sig, n_enrich, r_spear)
pA_legend   <- NULL
composite <- composite &
