...
QUAD_COLORS <- c("Reversed Up" = "#2563EB", "Reversed Down" = "#DC2626",
                 "Non-reversed" = "#FFB74D")
QUAD_BG     <- c("Reversed Up" = scales::alpha("#2563EB", 0.08),
                 "Reversed Down" = scales::alpha("#DC2626", 0.08),
                 "Non-reversed" = scales::alpha("#FFB74D", 0.08))
ENDPOINT_COLORS <- QUAD_COLORS
SIG_COLORS_HM  <- c("Both" = "#2E7D32", "Cancer" = "#4CAF50",
...
p <- ggplot() +
  ...
  annotate("text", x = X_BAR_L + (count_max / 2) * BAR_SCALE,
  ...
  coord_cartesian(xlim = c(0.0, X_BAR_MAX + 2.0),
                  ...
                  expand = FALSE) +
  labs(title = "Reversal Pattern Classification",
