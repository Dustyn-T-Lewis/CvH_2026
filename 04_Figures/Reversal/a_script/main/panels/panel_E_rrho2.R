...
rrho_obj <- RRHO2_initialize(
  list1, list2,
  labels          = c("Cancer_vs_Healthy", "Training_CR"),
  log10.ind       = TRUE,
...
if (length(na_rows) > 0 && length(na_cols) > 0) {
  ...
  mid <- floor(nr / 2)
  row_before <- 1:mid; row_after  <- (mid + 1):nr
  col_before <- 1:mid; col_after  <- (mid + 1):nc
}
...
message(sprintf("  Max -log10(p): UU=%.1f, DD=%.1f, UD=%.1f, DU=%.1f",
...
pE_heat <- ggplot(hmat_df, aes(x = row, y = col, fill = neg_log10_p)) +
  ...
  scale_fill_gradientn(
    ...
    guide    = guide_colorbar(
      ...
      title.theme = element_text(size = FIG_LEGEND_TITLE, face = "bold",
                                  color = "grey15"))
  ) +
  ...
  labs(
    ...
    subtitle = sprintf("Stratified hypergeometric | %d genes | peak = %.0f",
                        n_shared, max_val),
...
# --- Summary CSV ---
rrho2_meta <- tibble(
  quadrant = c("Exacerbated Up", "Exacerbated Down",
  ...
  n_ora_pathways = c(nrow(ora_exac_up), nrow(ora_exac_down),
