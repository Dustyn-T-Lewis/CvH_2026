# F07 Panel E: Unified ORA-DEP Chord Diagram -- CR Supplement Training
# Single chord connecting ORA-enriched pathways to CR DEPs
# (Training_CRE + Training_PLA + Supplement_Interaction, Pi < 0.05 in any).
#
...
FIGURE_LABEL       <- "CR Training"
RELEVANT_CONTRASTS <- c("Training_CRE", "Training_PLA", "Supplement_Interaction")
...
write_csv(
  ...
  file.path(DAT, "panel_E", "chord_cr_training_combined.csv")
)

write_csv(
  pw_direction %>% select(pathway_label, pathway_padj, pathway_db,
                            n_overlap, mean_logFC, direction),
  file.path(DAT, "panel_E", "chord_cr_training_pathways.csv")
)
...
ctr_col_lookup <- CONTRAST_COLORS_F07[ctr_lookup[proteins_ordered]]
...
render_chord <- function(filepath, ext) {
  ...
  # TRACK 1 (outermost, h=0.14): gene names | direction-colored arc + name
  ...
  y_cursor <- draw_legend(
    ...
    colors = CONTRAST_COLORS_F07[RELEVANT_CONTRASTS]
  )
