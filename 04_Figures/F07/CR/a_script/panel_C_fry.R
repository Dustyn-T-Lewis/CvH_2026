# F07 Panel C -- fry Rotation Test: Supplement Training Concordance Barcode
# Tests whether Training_CRE-significant protein sets respond concordantly
# in PLA subjects, using limma's fry gene-set rotation test.
#
# No circularity: Training_CRE (CRE_Post - CRE_Pre) and Training_PLA
# (PLA_Post - PLA_Pre) share no contrast terms (different subjects).
#
...
# CR model uses only CR subjects (CRE + PLA), not Healthy
cr_meta <- meta %>% filter(Group %in% c("CR_CRE", "CR_PLA"))
sample_cols <- cr_meta$Col_ID
...
cr_meta$Group_Time <- factor(paste0(cr_meta$Group_Time),
...
sig_pi <- dep_df %>%
  filter(pi_score_Training_CRE < 0.05, uniprot_id %in% imp_ids)

sets_pi <- list(
  up      = match(sig_pi$uniprot_id[sig_pi$logFC_Training_CRE > 0], imp_ids),
  down    = match(sig_pi$uniprot_id[sig_pi$logFC_Training_CRE < 0], imp_ids),
  up_ids  = sig_pi$uniprot_id[sig_pi$logFC_Training_CRE > 0],
  down_ids = sig_pi$uniprot_id[sig_pi$logFC_Training_CRE < 0]
)
...
driving_up <- dep_df %>%
  filter(uniprot_id %in% sets_pi$up_ids, uniprot_id %in% imp_ids,
         t_Training_PLA > 0) %>%
  transmute(gene, uniprot_id, set = "cre_up",
...
driving_dn <- dep_df %>%
  filter(uniprot_id %in% sets_pi$down_ids, uniprot_id %in% imp_ids,
         t_Training_PLA < 0) %>%
  transmute(gene, uniprot_id, set = "cre_down",
            t_training_cre = t_Training_CRE, t_training_pla = t_Training_PLA,
            logFC_Training_CRE, logFC_Training_PLA, pi_score_Training_CRE)
...
CRE_COLOR <- unname(CONTRAST_COLORS_F07["Training_CRE"])
PLA_COLOR <- unname(CONTRAST_COLORS_F07["Training_PLA"])
...
make_barcode <- function(t_df, in_col, es_col, fry_row, title, color,
  ...
  if (p_position == "bottom_left") {
    p_x <- n_all * 0.02; p_y <- -Inf; p_hjust <- 0; p_vjust <- -0.5
  } else {
    p_x <- n_all * 0.98; p_y <- Inf; p_hjust <- 1; p_vjust <- 1.3
  }
...
up_title <- sprintf("Tr.(CRE)-Up DEPs (Pi < 0.05, n = %d) \u2192 Tr.(PLA) ranked t",
                     length(sets_pi$up))
dn_title <- sprintf("Tr.(CRE)-Down DEPs (Pi < 0.05, n = %d) \u2192 Tr.(PLA) ranked t%s",
                     length(sets_pi$down),
                     if (fry_dn$PValue > 0.05) "  (n.s.)" else "")

# Use CRE color for both barcode curves (concordance from CRE perspective)
FIG_COLOR <- unname(CONTRAST_COLORS_F07["Training_CRE"])
...
p_t <- ggplot(t_rank, aes(x = rank, y = t_Training_PLA)) +
  geom_area(fill = scales::alpha(PLA_COLOR, 0.20),
            color = PLA_COLOR, linewidth = 0.3) +
  geom_hline(yintercept = 0, linetype = "dashed", linewidth = 0.3) +
  labs(x = sprintf("Protein rank by t(Training PLA)  [n = %d]", n_all),
       y = "t-stat") +
  scale_x_continuous(limits = c(1, n_all), expand = c(0.005, 0)) +
  FIG_THEME +
...
make_flanking_ora <- function(ora_df, set_label, bar_color) {
  ...
  bars <- ora_df %>%
    ...
    mutate(neg_log_padj = -log10(pmax(padj, 1e-20)),
           ...
           y = rev(row_number()),
           bar_h = 0.7,
           text_size = case_when(nchar(short_label) > 28 ~ 2.0,
...
pC_fry <- p1$es + p1$bc + p2$es + p2$bc + p_t +
  ...
  plot_layout(design = fry_design,
              ...
              widths  = c(3, 1.5)) +
  plot_annotation(
    title = "fry Gene-Set Rotation Test: Supplement Training Concordance",
...
if (!is.null(p_ora_up) || !is.null(p_ora_dn)) {
  ...
  message("F07 Panel C ORA (supp) done")
}

message("F07 Panel C (fry) done")
