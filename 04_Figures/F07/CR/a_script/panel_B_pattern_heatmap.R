# F07 Panel B: CR Training Concordance Pattern Heatmap
# Layout: [legends] [sig|heatmap|quad] [sankey] [stacked bars in striped bg]
# 3 groups: Concordant Up, Concordant Down, Discordant
# DEPs significant in any CR contrast (Training_CRE, Training_PLA, Supplement_Interaction)
...
sig_df <- dep_df %>%
  ...
  filter(pi_score_Training_CRE < 0.05 | pi_score_Training_PLA < 0.05 |
         pi_score_Supplement_Interaction < 0.05) %>%
  mutate(
    quadrant = case_when(
    ...
    sig_cat = case_when(
      ...
      pi_score_Supplement_Interaction < 0.05 ~ "Inter.",
      TRUE ~ "NS"
    )
  )
...
SIG_COLORS <- c("Both" = "#2E7D32", "Tr.(CRE)" = "#2166AC",
                "Tr.(PLA)" = "#D6604D", "Inter." = "#FF8F00", "NS" = "grey70")
...
sig_df <- sig_df %>%
  ...
  arrange(quadrant, pathway, desc(logFC_Training_CRE))
...
dom_quad <- sig_df %>%
  ...
  summarise(n = n(),
            lfc_sum = sum(abs(logFC_Training_CRE) + abs(logFC_Training_PLA)),
            .groups = "drop") %>%
  group_by(pathway) %>%
...
# 7. HEATMAP TILE DATA
# =============================================================================
fc_max <- max(abs(c(sig_df$logFC_Training_CRE, sig_df$logFC_Training_PLA)), na.rm = TRUE)
...
heat_tiles <- bind_rows(
  sig_df %>% transmute(x = X_COL1, y, w = TILE_W, h = ROW_H,
                        fill = lfc_to_color(logFC_Training_CRE, fc_max)),
  sig_df %>% transmute(x = X_COL2, y, w = TILE_W, h = ROW_H,
                        fill = lfc_to_color(logFC_Training_PLA, fc_max))
)
sig_tiles <- sig_df %>%
...
sig_leg_cats <- c("Both", "Tr.(CRE)", "Tr.(PLA)", "Inter.")
...
p <- ggplot() +
  ...
  coord_cartesian(xlim = c(-0.8, X_BAR_MAX + 1.0),
                  ...
                  expand = FALSE) +
  labs(title = "Supplement Training Response Patterns",
