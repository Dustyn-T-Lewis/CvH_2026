# F06 CRvH Panel B ORA: Scatter + Flanking ORA Bars Composite
# Threshold-free ORA on all proteins per reversal quadrant.
# Reversal framing: Cancer_vs_Healthy (x) vs Training_CR (y).
...
scatter_df <- dep_df %>%
  ...
  mutate(
    ...
    quadrant     = case_when(
      ...
      logFC_CvH < 0 & logFC_TR < 0 ~ "Exacerbated Down",
...
ora_q1 <- run_set_ora(scatter_df$gene[scatter_df$quadrant == "Exacerbated Up"],
                       "Exacerbated Up")
...
ora_q3 <- run_set_ora(scatter_df$gene[scatter_df$quadrant == "Exacerbated Down"],
                       "Exacerbated Down")
...
for (qn in c("Exacerbated Up", "Exacerbated Down",
             "Discordant (Cancer Down / Training Up)",
             "Discordant (Cancer Up / Training Down)"))
...
message("\nF06 CRvH Panel B ORA composite done")
