...
# CRvH_Baseline (x) vs CR_Training (y) at pathway level
...
fgsea_hg <- fgsea_hg %>%
  mutate(
    set_size = coalesce(size_CRvH_Baseline, size_CR_Training),
    sig_1 = !is.na(padj_CRvH_Baseline) & padj_CRvH_Baseline < 0.05,
    sig_2 = !is.na(padj_CR_Training) & padj_CR_Training < 0.05,
...
nes_cor_sig <- if (nrow(fgsea_sig) >= 3) {
  cor.test(fgsea_sig$NES_CRvH_Baseline,
    fgsea_sig$NES_CR_Training,
    method = "spearman"
  )
} else {
  NULL
...
n_q4 <- sum(fgsea_sig$NES_CRvH_Baseline > 0 & fgsea_sig$NES_CR_Training < 0)
...
pB <- ggplot(mapping = aes(x = NES_CRvH_Baseline, y = NES_CR_Training)) +
  ...
  geom_point(
    ...
    size = 1.4, fill = "grey70", color = "grey55", alpha = 0.40, stroke = 0.2
  ) +
  # Sig points
  geom_point(
  ...
  labs(
    title = "Pathway NES: Cancer Recovery Reversal",
  ...
  theme(
    ...
    legend.title      = element_text(size = FIG_LEGEND_TITLE + 1.5, face = "bold", color = "grey25"),
    legend.text       = element_text(size = FIG_LEGEND_TEXT + 1.5, color = "grey20"),
  ...
  guides(
    ...
    shape = guide_legend(
      ...
      keyheight = unit(6, "mm"),
      keywidth = unit(6, "mm"),
      override.aes = list(size = 4.5, fill = "grey50")
    ),
    size = guide_legend(
      nrow = 1, order = 2,
      keyheight = unit(6, "mm"),
...
export_df <- fgsea_hg %>%
  transmute(
    ...
    NES_CRvH_Baseline = round(NES_CRvH_Baseline, 3),
    NES_CR_Training = round(NES_CR_Training, 3),
    padj_CRvH_Baseline = signif(padj_CRvH_Baseline, 4),
    padj_CR_Training = signif(padj_CR_Training, 4),
