...
#   Reference = all proteins ranked by CR_Training t-statistic
...
dep_df <- dep_df %>%
  filter(!is.na(logFC_CRvH_Baseline), !is.na(logFC_CR_Training),
         !is.na(t_CR_Training), !is.na(t_CRvH_Baseline))
...
cancer_sig <- dep_df %>% filter(pi_score_CRvH_Baseline < 0.05)
up_genes <- cancer_sig$gene[cancer_sig$logFC_CRvH_Baseline > 0]
dn_genes <- cancer_sig$gene[cancer_sig$logFC_CRvH_Baseline < 0]
...
up_weights <- abs(all_ranked$t_CRvH_Baseline[up_ranks])
dn_weights <- abs(all_ranked$t_CRvH_Baseline[dn_ranks])
