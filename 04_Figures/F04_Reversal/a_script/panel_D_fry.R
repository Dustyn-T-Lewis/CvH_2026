...
#
# Test 1 (original): Cancer-sig gene sets tested on CR_Training contrast
#   "Do cancer-altered proteins reverse with training?"
#   Subsets to CR subjects only, blocks on Subject_ID
#
# Test 2 (reciprocal): Training-sig gene sets tested on CRvH_Baseline contrast
...
circ_r <- cor(dep_df$t_CRvH_Baseline, dep_df$t_CR_Training,
  use = "complete.obs"
)
  ...
  p_es <- ggplot(t_df, aes(x = rank, y = .data[[es_col]])) +
    ...
    theme(
      ...
      plot.title = element_text(
        size = 10, face = "bold", color = "grey15",
...
cm_tr <- makeContrasts(
  CR_Training = (CRE_T2 + PLA_T2) / 2 - (CRE_T1 + PLA_T1) / 2,
  levels = design_cr
)
...
sig_cancer <- dep_df %>%
  filter(pi_score_CRvH_Baseline < 0.05, uniprot_id %in% imp_ids_cr)

sets_cancer <- list(
  up = na.omit(match(sig_cancer$uniprot_id[sig_cancer$logFC_CRvH_Baseline > 0], imp_ids_cr)),
  down = na.omit(match(sig_cancer$uniprot_id[sig_cancer$logFC_CRvH_Baseline < 0], imp_ids_cr)),
  up_ids = sig_cancer$uniprot_id[sig_cancer$logFC_CRvH_Baseline > 0],
  down_ids = sig_cancer$uniprot_id[sig_cancer$logFC_CRvH_Baseline < 0]
)

message(sprintf(
  "  Cancer gene sets: up = %d, down = %d",
  length(sets_cancer$up), length(sets_cancer$down)
))

fry_c_up <- run_fry_generic(
  mat_cr, design_cr, cm_tr[, "CR_Training"],
...
message(sprintf(
  "  cancer_up  -> Training: fry %s, p = %.4f (exp Down) %s",
  fry_c_up$direction, fry_c_up$PValue,
  ifelse(fry_c_up$consistent, "OK", "UNEXPECTED")
))
message(sprintf(
  "  cancer_down -> Training: fry %s, p = %.4f (exp Up) %s",
  fry_c_dn$direction, fry_c_dn$PValue,
  ifelse(fry_c_dn$consistent, "OK", "UNEXPECTED")
))

# Barcode rank data (CR_Training)
t_rank_tr <- dep_df %>%
  filter(uniprot_id %in% imp_ids_cr, !is.na(t_CR_Training)) %>%
  arrange(desc(t_CR_Training)) %>%
  mutate(
    rank = row_number(),
...
t_rank_tr$es_up <- running_es(t_rank_tr$t_CR_Training, t_rank_tr$in_up)
t_rank_tr$es_down <- running_es(t_rank_tr$t_CR_Training, t_rank_tr$in_down)
n_tr <- nrow(t_rank_tr)

# Driving proteins (cancer-up reversed by training)
driving_up_t1 <- dep_df %>%
  filter(
    uniprot_id %in% sets_cancer$up_ids, uniprot_id %in% imp_ids_cr,
    t_CR_Training < 0
  ) %>%
  transmute(gene, uniprot_id,
    set = "cancer_up", test = "cancer_on_training",
    t_cancer = t_CRvH_Baseline, t_training = t_CR_Training,
    logFC_CRvH_Baseline, logFC_CR_Training, pi_score_CRvH_Baseline
  )
driving_dn_t1 <- dep_df %>%
  filter(
    uniprot_id %in% sets_cancer$down_ids, uniprot_id %in% imp_ids_cr,
    t_CR_Training > 0
  ) %>%
  transmute(gene, uniprot_id,
    set = "cancer_down", test = "cancer_on_training",
    t_cancer = t_CRvH_Baseline, t_training = t_CR_Training,
    logFC_CRvH_Baseline, logFC_CR_Training, pi_score_CRvH_Baseline
  )
...
ora_t1_up <- if (nrow(driving_up_t1) >= 5) {
  tryCatch(
    run_ora_deduplicated(
      genes = unique(driving_up_t1$gene), universe = all_genes,
      pathways = pw_collection, jaccard_cutoff = 0.5,
      min_size = 10, max_size = 500, padj_cutoff = 1
    ) %>%
...
ora_t1_dn <- if (nrow(driving_dn_t1) >= 5) {
  tryCatch(
    run_ora_deduplicated(
      genes = unique(driving_dn_t1$gene), universe = all_genes,
      pathways = pw_collection, jaccard_cutoff = 0.5,
      min_size = 10, max_size = 500, padj_cutoff = 1
    ) %>%
      mutate(pathway_label = clean_pathway_name(pathway)) %>%
...
cm_cvh <- makeContrasts(
  CRvH_Baseline = (CRE_T1 + PLA_T1) / 2 - H_T1,
  levels = design_all
)

# Training-sig gene sets
sig_training <- dep_df %>%
  filter(pi_score_CR_Training < 0.05, uniprot_id %in% imp_ids)

sets_training <- list(
  up = na.omit(match(sig_training$uniprot_id[sig_training$logFC_CR_Training > 0], imp_ids)),
  down = na.omit(match(sig_training$uniprot_id[sig_training$logFC_CR_Training < 0], imp_ids)),
  up_ids = sig_training$uniprot_id[sig_training$logFC_CR_Training > 0],
  down_ids = sig_training$uniprot_id[sig_training$logFC_CR_Training < 0]
)

message(sprintf(
  "  Training gene sets: up = %d, down = %d",
...
fry_t_up <- run_fry_generic(
  mat, design_all, cm_cvh[, "CRvH_Baseline"],
...
message(sprintf(
  "  training_up  -> Cancer: fry %s, p = %.4f (exp Down) %s",
  fry_t_up$direction, fry_t_up$PValue,
  ifelse(fry_t_up$consistent, "OK", "UNEXPECTED")
))
message(sprintf(
  "  training_down -> Cancer: fry %s, p = %.4f (exp Up) %s",
  fry_t_dn$direction, fry_t_dn$PValue,
  ifelse(fry_t_dn$consistent, "OK", "UNEXPECTED")
))

# Barcode rank data (CRvH_Baseline)
t_rank_cvh <- dep_df %>%
  filter(uniprot_id %in% imp_ids, !is.na(t_CRvH_Baseline)) %>%
  arrange(desc(t_CRvH_Baseline)) %>%
  mutate(
    rank = row_number(),
...
t_rank_cvh$es_up <- running_es(t_rank_cvh$t_CRvH_Baseline, t_rank_cvh$in_up)
t_rank_cvh$es_down <- running_es(t_rank_cvh$t_CRvH_Baseline, t_rank_cvh$in_down)
n_cvh <- nrow(t_rank_cvh)

# Driving proteins (training-up appearing cancer-down = reversed)
driving_up_t2 <- dep_df %>%
  filter(
    uniprot_id %in% sets_training$up_ids, uniprot_id %in% imp_ids,
    t_CRvH_Baseline < 0
  ) %>%
  transmute(gene, uniprot_id,
    set = "training_up", test = "training_on_cancer",
    t_cancer = t_CRvH_Baseline, t_training = t_CR_Training,
    logFC_CRvH_Baseline, logFC_CR_Training, pi_score_CR_Training
  )
driving_dn_t2 <- dep_df %>%
  filter(
    uniprot_id %in% sets_training$down_ids, uniprot_id %in% imp_ids,
    t_CRvH_Baseline > 0
  ) %>%
  transmute(gene, uniprot_id,
    set = "training_down", test = "training_on_cancer",
    t_cancer = t_CRvH_Baseline, t_training = t_CR_Training,
    logFC_CRvH_Baseline, logFC_CR_Training, pi_score_CR_Training
  )
...
ora_t2_up <- if (nrow(driving_up_t2) >= 5) {
  tryCatch(
    run_ora_deduplicated(
      genes = unique(driving_up_t2$gene), universe = all_genes,
      pathways = pw_collection, jaccard_cutoff = 0.5,
      min_size = 10, max_size = 500, padj_cutoff = 1
    ) %>%
      mutate(pathway_label = clean_pathway_name(pathway)) %>%
...
ora_t2_dn <- if (nrow(driving_dn_t2) >= 5) {
  tryCatch(
    run_ora_deduplicated(
      genes = unique(driving_dn_t2$gene), universe = all_genes,
      pathways = pw_collection, jaccard_cutoff = 0.5,
      min_size = 10, max_size = 500, padj_cutoff = 1
    ) %>%
      mutate(pathway_label = clean_pathway_name(pathway)) %>%
...
compute_neff <- function(mat_sub, idx, label) {
  ...
  n_eff <- n / vif
  message(sprintf(
    "  %s: n = %d, mean r = %.3f, n_eff = %.1f (%.0f%% of nominal)",
...
neff_df <- tibble(
  ...
  pct_of_nominal = round(100 * c(
    neff_c_up$n_eff, neff_c_dn$n_eff,
...
fry_all <- bind_rows(fry_c_up, fry_c_dn, fry_t_up, fry_t_dn) %>%
  mutate(
    ...
    mean_pairwise_r = c(
      neff_c_up$r_bar, neff_c_dn$r_bar,
...
p_t1 <- ggplot(t_rank_tr, aes(x = rank, y = t_CR_Training)) +
  geom_area(
    fill = scales::alpha(COLOR_TRAINING, 0.20),
  ...
  FIG_THEME +
  theme(
    axis.title.x = element_text(size = 7, face = "bold"), axis.title.y = element_blank(),
...
p2b <- make_barcode(t_rank_cvh, "in_down", "es_down", fry_t_dn,
  sprintf(
    "Training-Down (n = %d)%s", length(sets_training$down),
...
p_t2 <- ggplot(t_rank_cvh, aes(x = rank, y = t_CRvH_Baseline)) +
  geom_area(
    fill = scales::alpha(COLOR_CANCER, 0.20),
...
ggsave(file.path(RPT_PNG, "MAIN_F04_fry.png"), pD_fry,
  width = PANEL_W * 2, height = 175, units = "mm", dpi = 300
)
ggsave(file.path(RPT_PDF, "MAIN_F04_fry.pdf"), pD_fry,
