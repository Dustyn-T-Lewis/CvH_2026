# Figure 8 — Panel B: WGCNA Per-Module Triptych (CvH)
...
# Eigengene: paired Pre->Post for CRE and PLA, separate H_T1
# ORA bars use per-module x-scales (no artificial cap)
# Generates: panel_B_triptych_MAIN.pdf/.png
...
message("Panel B: WGCNA per-module triptych (CvH)...")

# --- Load data ---
z_scores  <- read_csv(file.path(SRC, "03_panel_B_heatmap_zscores.csv"), show_col_types = FALSE)
me_data   <- read_csv(file.path(SRC, "03_panel_B_eigengene_data.csv"), show_col_types = FALSE)
enrich    <- read_csv(file.path(SRC, "03_panel_B_triptych_enrichment.csv"), show_col_types = FALSE)
mod_bio   <- read_csv(file.path(SRC, "mod_bio_labels.csv"), show_col_types = FALSE)
lmm_audit <- read_csv(file.path(SRC, "wgcna_lmm_contrast_audit.csv"), show_col_types = FALSE)
...
# Order by module size (largest first)
mod_order <- mod_bio %>%
...
build_row <- function(mod, show_xlab = FALSE) {
  ...
  # -- Eigengene dynamics: paired lines for CRE and PLA, H_T1 as reference --
  me_mod <- me_data %>%
    filter(module == mod) %>%
    mutate(group_time = factor(group_time, levels = group_order))

  # Individual subject lines (paired within CRE, PLA)
  me_cre <- me_mod %>% filter(group_time %in% c("CRE_T1", "CRE_T2"))
  me_pla <- me_mod %>% filter(group_time %in% c("PLA_T1", "PLA_T2"))
  me_h   <- me_mod %>% filter(group_time == "H_T1")
  ...
  p_int <- lmm_stats %>% filter(module == mod, contrast == "Supplement_Interaction") %>% pull(p_bh)
  ...
  p_eigen <- ggplot() +
    # Individual paired lines (CRE)
    geom_line(data = me_cre, aes(x = group_time, y = eigengene, group = subject),
              color = GROUP_COLORS["CRE_T1"], alpha = 0.15, linewidth = 0.3) +
    # Individual paired lines (PLA)
    geom_line(data = me_pla, aes(x = group_time, y = eigengene, group = subject),
              color = GROUP_COLORS["PLA_T1"], alpha = 0.15, linewidth = 0.3) +
    # Group means + SE bars
    geom_errorbar(data = me_summary,
                  aes(x = group_time, ymin = mean_me - se_me, ymax = mean_me + se_me),
                  width = 0.15, linewidth = 0.5) +
    geom_point(data = me_summary,
               aes(x = group_time, y = mean_me, color = group_time), size = 3) +
    # Mean lines connecting CRE T1->T2 and PLA T1->T2
    geom_line(data = me_summary %>% filter(group_time %in% c("CRE_T1", "CRE_T2")),
              aes(x = group_time, y = mean_me, group = 1),
              color = GROUP_COLORS["CRE_T1"], linewidth = 0.9) +
    geom_line(data = me_summary %>% filter(group_time %in% c("PLA_T1", "PLA_T2")),
              aes(x = group_time, y = mean_me, group = 1),
              color = GROUP_COLORS["PLA_T1"], linewidth = 0.9) +
    scale_color_manual(values = GROUP_COLORS, guide = "none") +
    # Significance annotations
    annotate("text", x = 1.5, y = y_range[2] + y_span * 0.15,
             label = fmt_sig(p_cre), size = txt_sig, fontface = "bold",
             color = GROUP_COLORS["CRE_T1"]) +
    annotate("text", x = 3.5, y = y_range[1] - y_span * 0.15,
             label = fmt_sig(p_pla), size = txt_sig, fontface = "bold",
             color = GROUP_COLORS["PLA_T1"]) +
    # Cancer vs Healthy annotation (top-right)
    {if (length(p_cvh) > 0 && !is.na(p_cvh) && p_cvh < 0.05)
      annotate("text", x = 5, y = y_range[2] + y_span * 0.1,
               label = paste0("CvH:", fmt_sig(p_cvh)),
               size = txt_sig * 0.8, fontface = "bold", color = "grey40")
    } +
    labs(y = "Eigengene", x = NULL) +
    FIG_THEME +
    theme(
      axis.text.x  = if (show_xlab) element_text(size = txt_axis * 0.65, angle = 45, hjust = 1)
                      else element_blank(),
