# F03 Panel C: Pattern-Sorted Heatmap + Sankey + Scatterpie (single panel)
...
classify_pattern_f3 <- function(pi_CRE, pi_PLA, pi_int, threshold = 0.05) {
  dplyr::case_when(
    pi_CRE < threshold & pi_PLA < threshold &
      sign(logFC_CRE) == sign(logFC_PLA)        ~ "Shared",
    pi_CRE < threshold & pi_PLA >= threshold     ~ "CRE-specific",
    pi_CRE >= threshold & pi_PLA < threshold     ~ "PLA-specific",
...
expanded_df <- dep_df %>%
  ...
  mutate(
    logFC_CRE = logFC_Training_CRE,
    logFC_PLA = logFC_Training_PLA,
    pattern = dplyr::case_when(
      pi_score_Training_CRE < 0.05 & pi_score_Training_PLA < 0.05 &
        sign(logFC_Training_CRE) == sign(logFC_Training_PLA) ~ "Shared",
      pi_score_Training_CRE < 0.05 & pi_score_Training_PLA >= 0.05 ~ "CRE-specific",
      pi_score_Training_CRE >= 0.05 & pi_score_Training_PLA < 0.05 ~ "PLA-specific",
      pi_score_Supplement_Interaction < 0.05                        ~ "Interaction",
      TRUE ~ NA_character_
    )
  ) %>%
  filter(!is.na(pattern)) %>%
  mutate(pattern = factor(pattern, levels = PATTERN_ORDER_F3)) %>%
...
expanded_df <- expanded_df %>%
  mutate(sort_key = case_when(
    pattern %in% c("Shared", "CRE-specific") ~ logFC_Training_CRE,
    pattern == "PLA-specific"                 ~ logFC_Training_PLA,
    pattern == "Interaction"                  ~ abs(logFC_Training_CRE - logFC_Training_PLA)
  ))
...
gene_order <- links_1to1 %>%
  mutate(pat_idx = match(as.character(pattern), PATTERN_ORDER_F3)) %>%
...
n_pat_breaks <- length(PATTERN_ORDER_F3) - 1
gap_size     <- 1.2
...
HM_X_PLA <- 1.92
HM_HW    <- 0.45
ANNO_X   <- HM_X_PLA + HM_HW + 0.01 + 0.12
ANNO_HW  <- 0.12
S_X_PW   <- 5.2
S_PW_HW  <- 0.12
...
heatmap_df <- expanded_df %>%
  ...
  select(gene, y_ctr, ymin, ymax, logFC_Training_CRE, logFC_Training_PLA) %>%
  pivot_longer(cols = c(logFC_Training_CRE, logFC_Training_PLA),
               names_to = "contrast", values_to = "logFC") %>%
  mutate(x = ifelse(contrast == "logFC_Training_CRE", HM_X_CRE, HM_X_PLA))

fc_max <- max(abs(heatmap_df$logFC), na.rm = TRUE)

# Pattern annotation strip
anno_rects <- gene_bar_df %>%
  mutate(fill = PATTERN_COLS_F3[as.character(pattern)])
...
group_outlines <- cat_heatmap_pos %>%
  mutate(xmin = HM_X_CRE - HM_HW, xmax = ANNO_X + ANNO_HW,
         outline_col = PATTERN_COLS_F3[as.character(pattern)])
...
inline_label_df <- cat_heatmap_pos %>%
  mutate(
    x     = HM_X_CRE - HM_HW - 0.15,
    n     = ifelse(as.character(pattern) %in% names(pat_counts),
                   pat_counts[as.character(pattern)], 0L),
    label = sprintf("%s\n%s (n=%d)",
                    as.character(pattern),
                    PATTERN_SUBTITLES_F3[as.character(pattern)],
                    n),
    color = PATTERN_COLS_F3[as.character(pattern)]
  )
...
cat_totals <- links_1to1 %>%
  ...
  arrange(match(as.character(pattern), PATTERN_ORDER_F3))
...
for (ct in as.character(cat_totals$pattern)) {
  ...
  for (r in seq_len(nrow(ct_contribs))) {
    ...
    ) %>% mutate(pattern = ct, pathway = pw, fill_col = PATTERN_COLS_F3[ct])
  }
}
ribbons_df <- bind_rows(ribbon_list)

# --- Section 5: Scatterpie data (polygon wedges) ---
pie_counts <- links_1to1 %>%
  filter(pathway != "Other") %>%
  count(pathway, pattern, name = "n") %>%
  complete(pathway = active_pw_order,
           pattern = factor(PATTERN_ORDER_F3, levels = PATTERN_ORDER_F3),
           fill = list(n = 0L))
...
# Aspect ratio -> circular pies
x_range <- (PIE_X_END + 1.5) - (HM_X_CRE - HM_HW - 3.0)
...
pF <- ggplot() +
  ...
  scale_fill_identity() +
  # Column headers
  annotate("text", x = HM_X_CRE, y = header_y, label = "Training\n(CRE)",
           size = txt_pf, fontface = "bold", lineheight = 0.85) +
  annotate("text", x = HM_X_PLA, y = header_y, label = "Training\n(PLA)",
           size = txt_pf, fontface = "bold", lineheight = 0.85) +
  # Count axis
  ...
  scale_x_continuous(
    limits = c(HM_X_CRE - HM_HW - 3.0, PIE_X_END + 1.5), expand = c(0, 0)) +
  labs(x = NULL, y = NULL,
       title = "Significant Proteins: Functional Classification",
       subtitle = sprintf("%d proteins | %d GO Slim categories | 4 supplement-response patterns",
...
expanded_df %>%
  transmute(gene, pattern = as.character(pattern), sort_key = round(sort_key, 4),
            ...
            pi_score_CRE         = round(pi_score_Training_CRE, 6),
            pi_score_PLA         = round(pi_score_Training_PLA, 6),
            pi_score_Interaction = round(pi_score_Supplement_Interaction, 6),
            imputed) %>%
  arrange(match(pattern, PATTERN_ORDER_F3), desc(sort_key)) %>%
  write_csv(file.path(DAT, "panel_C", "pattern_classification.csv"))
...
links_1to1 %>%
  transmute(gene, pathway, go_slim_id = slim,
            ...
            logFC_Training_CRE = round(logFC_Training_CRE, 4),
            logFC_Training_PLA = round(logFC_Training_PLA, 4)) %>%
  arrange(match(pattern, PATTERN_ORDER_F3), pathway, gene) %>%
  write_csv(file.path(DAT, "panel_C", "sankey_links.csv"))

pie_counts %>%
  left_join(pie_totals, by = "pathway") %>%
  mutate(fraction = round(n / total, 4)) %>%
  select(pathway, pattern, n, total, fraction) %>%
  arrange(pathway, match(as.character(pattern), PATTERN_ORDER_F3)) %>%
  write_csv(file.path(DAT, "panel_C", "pie_data.csv"))

# --- Section 8: Export ---
ggsave(file.path(RPT, "panel_C_pattern_heatmap.pdf"), pF,
       width = PF_W, height = f_height, units = "mm", device = pdf_device, limitsize = FALSE)
ggsave(file.path(RPT, "panel_C_pattern_heatmap.png"), pF,
       width = PF_W, height = f_height, units = "mm", dpi = 300, limitsize = FALSE)

message("F03 Panel C done")
