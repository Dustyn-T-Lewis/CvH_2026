# F05/CR Panel F fry -- fry Rotation Test: Supplement Concordance Barcode
...
pi_up <- dep_df$uniprot_id[!is.na(dep_df$pi_score_Training_CRE) &
                             dep_df$pi_score_Training_CRE < 0.05 &
                             dep_df$logFC_Training_CRE > 0]
p_up  <- dep_df$uniprot_id[!is.na(dep_df$P.Value_Training_CRE) &
                             dep_df$P.Value_Training_CRE < 0.05 &
                             dep_df$logFC_Training_CRE > 0]
pi_dn <- dep_df$uniprot_id[!is.na(dep_df$pi_score_Training_CRE) &
                             dep_df$pi_score_Training_CRE < 0.05 &
                             dep_df$logFC_Training_CRE < 0]
p_dn  <- dep_df$uniprot_id[!is.na(dep_df$P.Value_Training_CRE) &
                             dep_df$P.Value_Training_CRE < 0.05 &
                             dep_df$logFC_Training_CRE < 0]
...
# -- Step 6: Run fry -- 2 configs x 2 sets ------------------------------------
...
pF_fry <- wrap_elements(p1) / wrap_elements(p2) / p_t +
  ...
  plot_annotation(
    ...
    subtitle = sprintf("Tr.(CRE) Up set (n = %d) -> Tr.(PLA) | fry p = %.3f | missForest-imputed (n = %d) | dupCor = %.3f",
...
message("F05/CR Panel F (fry) done")
