# F05/CR Panel E: RRHO2 Concordance Map + Per-Quadrant ORA (CRE vs PLA)
...
pE_heat <- ggplot(hmat_df, aes(x = row, y = col, fill = neg_log10_p)) +
  ...
  labs(
    ...
    subtitle = sprintf("Stratified hypergeometric | %d shared genes | No MTC (standard per Cahill et al. 2018)",
                        n_shared),
...
# Save RRHO heatmap
ggsave(file.path(RPT, "panel_E_RRHO_MAIN.pdf"), pE_heat,
       width = PE_W, height = PE_W, units = "mm", device = pdf_device)
ggsave(file.path(RPT, "panel_E_RRHO_MAIN.png"), pE_heat,
...
for (qm in quad_meta) {
  ...
  ggsave(file.path(RPT, sprintf("panel_E_ORA_%s_SUPP.pdf", qm$slug)), p,
         width = ORA_W, height = h, units = "mm", device = pdf_device)
  ggsave(file.path(RPT, sprintf("panel_E_ORA_%s_SUPP.png", qm$slug)), p,
         width = ORA_W, height = h, units = "mm", dpi = 300, bg = "transparent")
}

# --- Ranking metric sensitivity check
message("  Running RRHO2 ranking metric sensitivity check...")

alt_df <- dep_df %>%
  transmute(
    gene,
    alt_cre = sign(logFC_Training_CRE) * -log10(pmax(adj.P.Val_Training_CRE, 1e-300)),
    alt_pla = sign(logFC_Training_PLA) * -log10(pmax(adj.P.Val_Training_PLA, 1e-300))
  ) %>%
  filter(!is.na(alt_cre) & !is.na(alt_pla)) %>%
  distinct(gene, .keep_all = TRUE)

alt_list1 <- data.frame(gene = alt_df$gene, score = alt_df$alt_cre, stringsAsFactors = FALSE)
alt_list2 <- data.frame(gene = alt_df$gene, score = alt_df$alt_pla, stringsAsFactors = FALSE)

rrho_alt <- RRHO2_initialize(
  alt_list1, alt_list2,
  labels          = c("Training (CRE)", "Training (PLA)"),
  log10.ind       = TRUE,
  multipleTesting = "none",
...
alt_hotspot <- list(
  UU = rrho_alt$genelist_uu$gene_list_overlap_uu,
  DD = rrho_alt$genelist_dd$gene_list_overlap_dd,
  UD = rrho_alt$genelist_ud$gene_list_overlap_ud,
  DU = rrho_alt$genelist_du$gene_list_overlap_du
)
...
sensitivity_df <- tibble(
  quadrant     = c("UU", "DD", "UD", "DU"),
  n_primary    = c(n_UU, n_DD, n_UD, n_DU),
  n_alternative = vapply(alt_hotspot, length, integer(1)),
  n_intersect  = mapply(function(a, b) length(intersect(a, b)),
                        hotspot_genes, alt_hotspot),
  n_union      = mapply(function(a, b) length(union(a, b)),
                        hotspot_genes, alt_hotspot),
  jaccard      = mapply(jaccard, hotspot_genes, alt_hotspot),
  ranking_primary     = "t_statistic",
  ranking_alternative = "sign_logFC_neg_log10_padj"
)

write_csv(sensitivity_df, file.path(DAT, "panel_E", "rrho2_sensitivity.csv"))
message(sprintf("  Sensitivity: Jaccard UU=%.3f, DD=%.3f, UD=%.3f, DU=%.3f",
                sensitivity_df$jaccard[1], sensitivity_df$jaccard[2],
                sensitivity_df$jaccard[3], sensitivity_df$jaccard[4]))
