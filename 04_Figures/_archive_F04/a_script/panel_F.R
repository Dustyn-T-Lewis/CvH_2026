# F04 Panel F — fry Rotation Test: Cancer Recovery Reversal Barcode
# Tests whether Cancer_vs_Healthy-significant proteins show reversal
# under Training_CR, using limma's fry rotation framework.
#
# No circularity: Cancer_vs_Healthy (CR_T1 vs H_T1) and Training_CR
# (CR_T2 vs CR_T1) share CR_T1 in opposite directions.
# This is the standard paired pre-post design — not circular.
...
# CRvH model uses CR (CRE+PLA) + H subjects
# Training_CR contrast: CR_T2 - CR_T1
crvh_meta <- meta %>% filter(Group %in% c("CR_CRE", "CR_PLA", "PPS"))
crvh_samples <- crvh_meta$Col_ID
mat_crvh <- mat_imp[, intersect(crvh_samples, colnames(mat_imp))]
crvh_meta <- crvh_meta %>% filter(Col_ID %in% colnames(mat_crvh))

n_imp <- nrow(mat_crvh)
message(sprintf("CRvH matrix: %d proteins x %d samples", n_imp, ncol(mat_crvh)))

# -- Step 3: Design matrix + duplicateCorrelation -----------------------------
# Group_Time levels for CRvH: CRE_T1, CRE_T2, PLA_T1, PLA_T2, H_T1
crvh_meta$Group_Time <- factor(crvh_meta$Group_Time)
design <- model.matrix(~ 0 + Group_Time, data = crvh_meta)
colnames(design) <- gsub("^Group_Time", "", colnames(design))

block_id <- sub("_T[12]$", "", crvh_meta$Col_ID)

corfit_imp <- duplicateCorrelation(mat_crvh, design, block = block_id)
cor_imp <- corfit_imp$consensus.correlation
message(sprintf("Within-subject cor: %.4f", cor_imp))

# Training_CR = average(CR_T2) - average(CR_T1) where CR = CRE + PLA
# = (CRE_T2 + PLA_T2)/2 - (CRE_T1 + PLA_T1)/2
avail_levels <- colnames(design)
cm_formula <- "(CRE_T2 + PLA_T2)/2 - (CRE_T1 + PLA_T1)/2"
if (all(c("CRE_T2", "PLA_T2", "CRE_T1", "PLA_T1") %in% avail_levels)) {
  cm <- makeContrasts(
    Training_CR = (CRE_T2 + PLA_T2)/2 - (CRE_T1 + PLA_T1)/2,
    levels = design
  )
} else {
  stop("Required design levels not found: ", paste(avail_levels, collapse = ", "))
}

# -- Step 4: Define Cancer_vs_Healthy gene sets --------------------------------
imp_ids <- rownames(mat_crvh)
...
# -- Step 5: Run fry -----------------------------------------------------------
# For reversal: cancer_up proteins should go Down under training → expect "Down"
# Cancer_down proteins should go Up under training → expect "Up"
...
fry_all <- bind_rows(
  run_fry(mat_crvh, sets_pi, design, cm, block_id, cor_imp, "Imp_Pi"),
  run_fry(mat_crvh, sets_p,  design, cm, block_id, cor_imp, "Imp_P")
) %>%
  mutate(
    # Reversal: cancer_up → expect Down in training, cancer_down → expect Up
    expected = ifelse(set == "cvh_up", "Down", "Up"),
...
# -- Step 6: Driving proteins (concordant with reversal) -----------------------
driving_df <- bind_rows(
  dep_df %>%
    filter(uniprot_id %in% sets_pi$up_ids, uniprot_id %in% imp_ids,
           t_Training_CR < 0) %>%  # cancer_up reversed by training (t < 0)
  ...
  dep_df %>%
    filter(uniprot_id %in% sets_pi$down_ids, uniprot_id %in% imp_ids,
           t_Training_CR > 0) %>%  # cancer_down reversed by training (t > 0)
    transmute(gene, uniprot_id, set = "cvh_down",
              t_cancer_vs_healthy = t_Cancer_vs_Healthy, t_training_cr = t_Training_CR,
              logFC_Cancer_vs_Healthy, logFC_Training_CR, pi_score_Cancer_vs_Healthy)
)
write_csv(driving_df, file.path(DAT, "panel_F_fry", "driving_proteins.csv"))
message(sprintf("Driving proteins (reversal): %d", nrow(driving_df)))
...
# Cancer-Up → Training CR (expect Down = reversal)
p1 <- make_barcode(t_rank, "in_up", "es_up", fry_up,
                    "Cancer-Up -> Training CR  (expect Down = reversal)", "#D6604D")

# Cancer-Down → Training CR (expect Up = reversal)
p2 <- make_barcode(t_rank, "in_down", "es_down", fry_dn,
                    "Cancer-Down -> Training CR  (expect Up = reversal)", "#4393C3")

p_t <- ggplot(t_rank, aes(x = rank, y = t_Training_CR)) +
  geom_area(fill = "grey85", color = "grey60", linewidth = 0.3) +
  geom_hline(yintercept = 0, linetype = "dashed", linewidth = 0.3) +
  labs(x = sprintf("Protein rank by t(Training CR)  [%d proteins]", n_all),
       y = "t-stat") +
  scale_x_continuous(limits = c(1, n_all), expand = c(0.005, 0)) +
  FIG_THEME +
...
pF_fry <- wrap_elements(p1) / wrap_elements(p2) / p_t +
  ...
  plot_annotation(
    title = "fry Rotation Test: Cancer Recovery Reversal",
