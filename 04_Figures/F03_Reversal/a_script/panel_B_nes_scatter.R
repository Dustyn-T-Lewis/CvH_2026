...
nes_cor_all <- cor.test(fgsea_hg$NES_CRvH_Baseline,
                         fgsea_hg$NES_CR_Training, method = "spearman")
...
nes_cor_sig <- if (nrow(fgsea_sig) >= 3) {
  cor.test(fgsea_sig$NES_CRvH_Baseline,
           fgsea_sig$NES_CR_Training, method = "spearman")
} else NULL

nes_lim <- max(abs(c(fgsea_hg$NES_CRvH_Baseline,
                      fgsea_hg$NES_CR_Training))) * 1.15
...
label_pw <- fgsea_sig %>%
  mutate(nes_mag = abs(NES_CRvH_Baseline) + abs(NES_CR_Training)) %>%
