# F03/CRvH — Panel B: DEP Rank Location (Barcode Plot)
...
# Normalize density per contrast: peak = 1.0
dens_list <- dens_list |>
  group_by(contrast) |>
...
ggsave(file.path(RPT_PDF, "panel_B_barcode_MAIN.pdf"), pB,
       width = PD_W, height = PD_H, units = "mm", device = pdf_device)
ggsave(file.path(RPT_PNG, "panel_B_barcode_MAIN.png"), pB,
       width = PD_W, height = PD_H, units = "mm", dpi = 300)

cat("F03/CRvH Panel B done.\n")
