# F03/CRvH — Panel C: DEP Contrast Overlap (UpSet Plot)
...
a <- sig_sets[[CONTRASTS[1]]]; b <- sig_sets[[CONTRASTS[2]]]
...
overlap_df <- data.frame(
  set_A = CONTRASTS[1], set_B = CONTRASTS[2],
...
if (n_sig_overlaps > 0) {
  top_or_txt <- sprintf("CR vs H <-> Tr.(CR) overlap: OR=%.1f, %s",
                        overlap_df$odds_ratio[1], fmt_p(overlap_df$p_bh[1]))
} else {
  top_or_txt <- "No significant overlap"
}
...
pC_bars <- ggplot(bar_long, aes(x, count, fill = direction)) +
  ...
  labs(y = "Intersection\nsize",
       title = "Contrast Overlap (UpSet) — CRvH",
       subtitle = sprintf("Pi-score DEPs | %s", top_or_txt),
       tag = "C") +
  FIG_THEME +
...
pC <- (pC_bars / pC_dots) + plot_layout(heights = c(0.75, 0.25))

ggsave(file.path(RPT_PDF, "panel_C_upset_MAIN.pdf"), pC,
       width = PC_W, height = PC_H, units = "mm", device = pdf_device)
ggsave(file.path(RPT_PNG, "panel_C_upset_MAIN.png"), pC,
       width = PC_W, height = PC_H, units = "mm", dpi = 300)

message("F03/CRvH Panel C (upset, Pi-score) done")
