# F03/CR — Panel A: DEPs per Contrast (Pseudo-log Stacked Bar)
# CR model: 4 contrasts from 03_combined_results_CR.csv
# Outputs: pA_cr (ggplot object), audit_panel_A_dep_fraction_ci.csv
#
# CONTRACT — downstream scripts depend on these objects remaining in env:
...
ggsave(file.path(RPT, "panel_A_dep_counts_MAIN.pdf"), pA_cr,
       width = PA_W, height = PA_H, units = "mm", device = pdf_device)
ggsave(file.path(RPT, "panel_A_dep_counts_MAIN.png"), pA_cr,
       width = PA_W, height = PA_H, units = "mm", dpi = 300)

cat("F03/CR Panel A done.\n")
