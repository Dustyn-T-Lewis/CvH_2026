# F03/CR — Panel B: DEP Rank Location (Barcode Plot)
...
# Outputs: pB_cr (ggplot object), panel_B_barcode.pdf/.png
...
pB_cr <- ggplot() +
  ...
  labs(
    title    = "DEP Rank Location (CR Model)",
...
ggsave(file.path(RPT, "panel_B_barcode_MAIN.pdf"), pB_cr,
       width = PD_W, height = PD_H, units = "mm", device = pdf_device)
ggsave(file.path(RPT, "panel_B_barcode_MAIN.png"), pB_cr,
       width = PD_W, height = PD_H, units = "mm", dpi = 300)

cat("F03/CR Panel B done.\n")
