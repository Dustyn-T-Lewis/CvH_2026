...
DISPLAY_CONTRASTS <- c("Baseline_Supplement", "Training_CRE",
...
bg_rects_c <- lapply(seq_along(DISPLAY_CONTRASTS), function(i) {
  annotate("rect",
           xmin = i - 0.5, xmax = i + 0.5,
           ymin = -Inf, ymax = Inf,
           fill = CONTRAST_COLORS[DISPLAY_CONTRASTS[i]], alpha = 0.20,
...
pD <- ggplot(count_df, aes(x = contrast, y = fraction * 100, fill = direction)) +
  ...
  labs(title = "Fraction of Pathways Enriched Up/Down (CR)",
...
cat("F03/CR Supp: fGSEA faceted done.\n")
