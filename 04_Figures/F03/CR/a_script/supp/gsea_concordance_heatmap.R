# F03/CR Supplementary: Cross-Contrast fGSEA Concordance Heatmap
# NES direction x significance across 4 CR contrasts for pathways
# significant in at least one contrast.
# Reads fGSEA cache from panel_C (must run panel_C.R first).
...
RPT <- "04_Figures/F03/CR/b_reports/supp"
DAT <- "04_Figures/F03/CR/c_data/supp"
...
fgsea_df <- read_csv("04_Figures/F03/CR/c_data/01_panel_C_fgsea_results.csv",
...
# Contrast ordering
ctr_order <- c("Baseline_Supplement", "Training_CRE",
...
# Column labels
col_labels <- c("BL(CRE\u2013PLA)", "Tr.(CRE)", "Tr.(PLA)", "CRE\u2013PLA")
...
ht <- Heatmap(
  ...
  column_title = "Cross-Contrast fGSEA Concordance (CR Model)",
...
cat("F03/CR Supp: GSEA concordance heatmap done.\n")
