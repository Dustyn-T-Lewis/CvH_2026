...
write.csv(audit_G, file.path(DAT, "panel_G_grip.csv"), row.names = FALSE)

y_max_left <- max(subj_long$grip, na.rm = TRUE)

pG_left <- ggplot(subj_long, aes(x = supp_time, y = grip, fill = supp_time)) +
  ...
  labs(title = "Grip Strength", subtitle = full_sub,
       y = "Grip Strength (lbs)", x = NULL, tag = "G") +
  FIG_THEME +
...
pG_right <- ggplot(subj, aes(x = supp, y = delta_grip, fill = supp)) +
...
pG <- (pG_left | pG_right) + plot_layout(widths = c(0.65, 0.35))

ggsave(file.path(RPT, "panel_G_grip_SUPP.pdf"), pG,
       width = PW, height = PH, units = "mm", device = get_pdf_device())
ggsave(file.path(RPT, "panel_G_grip_SUPP.png"), pG,
       width = PW, height = PH, units = "mm", dpi = 300)
cat("F01 Supp Panel G done\n")
