...
# --- Module-trait contrasts (pooled CR vs Ctl design) ---
# Each module eigengene is fit ME ~ group_time + (1 | subject); the random
# subject intercept absorbs the paired T1/T2 structure so pooling CR across
# timepoints and supplements does not inflate n. Contrasts:
#   CRvH_Baseline   pooled cancer survivors (all four CR group_times) vs healthy
#   CR_Training T2 vs T1 within CR (supplements pooled)
...
gt_levels <- c("CRE_T1", "CRE_T2", "PLA_T1", "PLA_T2", "H_T1")
gt_levels <- intersect(gt_levels, unique(meta$group_time))
crvh_contrasts <- list(
  CRvH_Baseline = c(0.25, 0.25, 0.25, 0.25, -1),
  CR_Training = c(-0.5, 0.5, -0.5, 0.5, 0)
)
crvh_contrasts <- lapply(crvh_contrasts, function(v) v[seq_along(gt_levels)])

lmm_df <- if (length(gt_levels) >= 4) {
  fit_lmm_contrasts("CRvCtl", gt_levels, crvh_contrasts)
} else {
  tibble()
}
if (nrow(lmm_df) > 0) {
  ...
  message(sprintf(
    "LMM audit: %d tests across %d modules",
...
# Trajectory table: pooled CR (T1, T2) + Ctl group-mean eigengenes for panel B
