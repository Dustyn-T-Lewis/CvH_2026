#!/usr/bin/env Rscript
# =============================================================================
# 01_run_dep.R  --  CvH Stage 03: Differential abundance (native limma)
#
# One fit on the NON-imputed normalized matrix (limma handles per-protein NAs):
...
#   CR_Training   = CR_post - CR_pre   (within-subject, paired recovery)
...
res <- compute_pi_scores(dal$results, pi_thresh = 0.05) |> map(~ left_join(.x, ann, by = "uniprot_id"))
