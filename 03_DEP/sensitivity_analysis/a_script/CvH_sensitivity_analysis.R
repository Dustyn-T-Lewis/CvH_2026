...
randna <- setNames(mar_mnar$class == "MNAR", mar_mnar$uniprot_id)
...
run_pipeline <- function(dal, norm_method, imp_spec, randna_vec) {
  ...
  dupcor <- tryCatch(
    ...
    error = function(e) {
      warning(sprintf("[%s_%s] duplicateCorrelation failed: %s -> correlation = 0",
...
for (lab in method_names) {
  for (cname in names(CONTRASTS)) {
    ...
    tt$pi_pval  <- rank(-tt$pi_score) / (nrow(tt) + 1)
...
p_criteria <- ggplot(dep_criteria, aes(x = method, y = n_total, fill = criterion)) +
  ...
  labs(x = NULL, y = "Total DEPs",
       ...
       subtitle = "Capital Pi = P^|logFC| < 0.05 (Xiao et al. 2014, Bioinformatics 30:801)",
       fill = "Criterion") +
  theme_minimal(base_size = 9) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        strip.text = element_text(face = "bold"))
