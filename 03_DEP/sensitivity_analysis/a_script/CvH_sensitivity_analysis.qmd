...
data_dir   <- file.path(here::here(), "03_DEP", "sensitivity_analysis", "c_data")
report_dir <- file.path(here::here(), "03_DEP", "sensitivity_analysis", "b_reports")
```
...
approach recommended by Välikangäs et al. (2018). This is the
...
to perform poorly on this dataset (Välikangäs et al., 2018).
...
`duplicateCorrelation()`, and `eBayes(robust = TRUE, trend = TRUE)` (Ritchie et al., 2015). All
...
a clearer view: rho captures global agreement across all
...
```{r top-pipeline}
top_pipe <- sensitivity %>%
  arrange(desc(mean_spearman)) %>%
  slice(1)
```

Based on the mean Spearman rho across all contrasts, the top-ranked
pipeline is `r top_pipe$method` (rho = `r round(top_pipe$mean_spearman, 4)`).

The top-ranked pipeline, `r top_pipe$method` (mean Spearman rho = `r round(top_pipe$mean_spearman, 4)`), achieves the highest concordance with the consensus ranking across all three contrasts. The narrow spread of concordance values across all 12 pipelines (all rho > 0.95) confirms that the choice of normalization and imputation has minimal impact on the relative protein rankings. This high concordance supports confidence in the differential expression results regardless of which specific pipeline is selected.

For the production analysis (Stage 04), we proceed with cycloess normalization on non-imputed data. While `r top_pipe$method` ranks highest in concordance, the practical difference between pipelines is negligible. Cycloess was selected as the normalization method based on the composite quality scoring in Stage 01 (PCV, PMAD, COR metrics), and using non-imputed data allows limma to handle missing values natively through its `na.rm` machinery in `arrayWeights`. This avoids introducing imputation artifacts while maintaining statistical power through the empirical Bayes framework.
...
3. Välikangäs T, Suomi T, Elo LL (2018). A systematic evaluation of
